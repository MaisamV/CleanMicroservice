...
inline fun <reified TThrowable: Throwable, reified TError: Any> OpenAPIPipelineResponseContext<*>.catch(throwable: TThrowable, noinline body: suspend OpenAPIPipelineResponseContext<TError>.() -> Unit) {
}
...
inline fun <reified TParams : Any, reified TResponse : Any, reified TRequest : Any> NormalOpenAPIRoute.route(
    ...
    return method(method).apply { modules.forEach { provider.registerModule(it, it::class.starProjectedType) } }
...
inline fun <reified TParams : Any, reified TResponse : Any, reified TRequest : Any> NormalOpenAPIRoute.handle(
    ...
    val paramsType = getKType<TParams>()
    val responseType = getKType<TResponse>()
    val requestType = getKType<TRequest>()
    return preHandle<TParams, TResponse, TRequest, NormalOpenAPIRoute>(exampleResponse, exampleRequest) {
...
inline fun <reified TParams : Any, reified TResponse : Any> NormalOpenAPIRoute.handle(
    ...
    val paramsType = getKType<TParams>()
    val responseType = getKType<TResponse>()
    return preHandle<TParams, TResponse, Unit, NormalOpenAPIRoute>(exampleResponse, Unit) {
