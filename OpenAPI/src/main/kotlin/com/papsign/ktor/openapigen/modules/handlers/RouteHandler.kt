...
import com.papsign.ktor.openapigen.model.OperationModelFactory
...
object RouteHandler: HandlerModule {
    ...
    var factory: IOperationModelFactory = OperationModelFactory

    override fun configure(factory: IOperationModelFactory, apiGen: OpenAPIGen, provider: ModuleProvider<*>) {
        ...
        apiGen.api.paths.getOrPut(path) { PathItemModel() }.also {pathItem ->
            methods.forEach {
                ...
                val op = pathItem.getOrPut(name) { factory.create() } as OperationModel
