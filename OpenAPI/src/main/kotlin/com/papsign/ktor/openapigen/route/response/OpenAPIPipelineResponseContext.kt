...
suspend inline fun <reified TResponse : Any, reified TError: Any> OpenAPIPipelineResponseContext<TResponse>.respondError(code: HttpStatusCode, error: TError) {
    val statusCode = route.provider.ofType<StatusProvider>().lastOrNull()?.getStatusForType(getKType<TResponse>()) ?: TResponse::class.findAnnotation<Response>()?.statusCode?.let { HttpStatusCode.fromValue(it) } ?: code
    responder.respond(statusCode, error as Any, pipeline)
}
